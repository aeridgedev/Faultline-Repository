...
const _DRILL_CLASSES  := [Constants.DrillClass.PRECISION, Constants.DrillClass.BURST,
						   Constants.DrillClass.THERMAL,  Constants.DrillClass.RESONANCE]
const _WEAPON_CLASSES := [Constants.WeaponClass.DAGGERS, Constants.WeaponClass.SWORDS,
						   Constants.WeaponClass.HAMMERS, Constants.WeaponClass.SPEARS,
						   Constants.WeaponClass.AXES]
const _ARMOR_CLASSES  := [Constants.ArmorClass.TITAN,    Constants.ArmorClass.HELLFORGE,
						   Constants.ArmorClass.TEMPEST,  Constants.ArmorClass.ECHO,
						   Constants.ArmorClass.EXPEDITION]


## Returns a Dictionary: {type: String, item_class: int, tier: Constants.Tier}
## or null if the roll somehow produces nothing (shouldn't happen).
static func roll(layer: Constants.Layer) -> Dictionary:
	...
	match category:
		"drill":
			return {
				"type": "drill",
				"item_class": _DRILL_CLASSES[randi() % _DRILL_CLASSES.size()],
				"tier": tier,
			}
		"weapon":
			return {
				"type": "weapon",
				"item_class": _WEAPON_CLASSES[randi() % _WEAPON_CLASSES.size()],
				"tier": tier,
			}
		"armor":
			return {
				"type": "armor",
				"item_class": _ARMOR_CLASSES[randi() % _ARMOR_CLASSES.size()],
				"tier": tier,
			}
	return {}


static func _roll_tier(rarity_weights: Dictionary) -> Constants.Tier:
	var weight_map := {
